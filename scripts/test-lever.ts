/**
 * Lever ATS test script.
 * Route: Steel.dev cloud browser (useProxy=true, solveCaptcha=true)
 *
 * Run:
 *   npx @dotenvx/dotenvx run -- npx tsx scripts/test-lever.ts [url]
 *
 * Default URL is a Lever job application. Pass any jobs.lever.co URL as arg.
 *
 * Success detection: Lever stays on /apply URL after submit — detected by
 * resume PDF "Success!" text remaining in the confirmation view.
 */
...
const TEST_URLS = process.argv.slice(2).length
  ...
  : [
      'https://jobs.lever.co/appcues-2/3aa44167-f64a-443d-bc02-016eea55399d/apply',
    ]
...
async function main() {
  ...
  for (const url of TEST_URLS) {
    const applicationId = `test-lever-${nanoid(8)}`
    console.log(`\n${'='.repeat(60)}`)
    console.log(`Application ID: ${applicationId}`)
    console.log(`URL: ${url}`)
    console.log('ATS: Lever → Steel.dev cloud browser')
