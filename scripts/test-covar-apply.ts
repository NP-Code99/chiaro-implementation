/**
 * One-off apply test for CoVar (Greenhouse board).
 * Run: npx tsx scripts/test-covar-apply.ts
...
const TARGET_URL = 'https://job-boards.greenhouse.io/covar/jobs/5097883007'
...
async function main() {
  ...
  const profile = {
    ...
    bio: 'Full-stack software engineer with 4 years of experience building scalable web applications. Expertise in TypeScript, React, Node.js, PostgreSQL, and AWS. Strong background in distributed systems and developer tooling.',
    applicationPassword: 'Chiaro2024!!',
  }

  const { browserApply } = await import('../src/lib/browserApply')

  const applicationId = `covar-test-${nanoid(8)}`
  console.log(`\nApplication ID : ${applicationId}`)
  console.log(`URL            : ${TARGET_URL}`)
  console.log('Live browser   : https://app.steel.dev/sessions\n')
